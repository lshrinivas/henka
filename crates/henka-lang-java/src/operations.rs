            ...
            params_schema: json!({
                ...
                "properties": {
                    ...
                    "limit": {
                        ...
                        "default": lsp::DEFAULT_SYMBOL_SEARCH_LIMIT
                    }
                }
            }),
        }
...
impl Operation for DescribeSymbolOp {
    ...
    async fn run(
        ...
        let out = lsp::hover_to_query(result).map_err(backend)?;
        Ok(OperationOutcome::Query(out))
    }
}
...
impl Operation for FileOutlineOp {
    ...
    async fn run(
        ...
        let out = lsp::document_symbols_to_query(result, &source(session, req), &uri)
            .map_err(backend)?;
        Ok(OperationOutcome::Query(out))
    }
}
...
/// isn't opened.
async fn prepare_for_call_query(session: &JdtlsSession, item: &Value) -> CoreResult<()> {
    session.ensure_indexed().await.map_err(backend)?;
    if let Some(uri) = item.get("uri").and_then(Value::as_str) {
        let _ = session.ensure_open(&lsp::uri_to_path(uri)).await;
    }
    Ok(())
}
...
impl Operation for IncomingCallsOp {
    ...
    async fn run(
        ...
        let out = lsp::incoming_calls_to_query(result, &source(session, req)).map_err(backend)?;
        Ok(OperationOutcome::Query(out))
    }
    fn route(&self, params: &Value) -> LanguageRoute {
        lsp::call_hierarchy_item_route(params)
    }
...
impl Operation for OutgoingCallsOp {
    ...
    async fn run(
        ...
        let out = lsp::outgoing_calls_to_query(result, &source(session, req), called_from)
            .map_err(backend)?;
        Ok(OperationOutcome::Query(out))
    }
    fn route(&self, params: &Value) -> LanguageRoute {
...
mod tests {
    ...
    #[test]
    fn file_outline_targets_a_whole_file() {
        let d = FileOutlineOp.descriptor();
        assert_eq!(d.id, "file-outline");
        assert_eq!(d.kind, OperationKind::Query);
        assert_eq!(d.target, TargetKind::File);
    }

    #[test]
    fn incoming_calls_require_an_item() {
        let d = IncomingCallsOp.descriptor();
        assert_eq!(d.id, "incoming-calls");
        // The item carries the location, so no coordinate is asked for.
        assert_eq!(d.target, TargetKind::Project);
        assert_eq!(d.params_schema["required"], json!(["item"]));

        // A call with no item runs nothing rather than guessing.
        let req = OperationRequest {
            target: Target::Project,
            params: json!({}),
        };
        assert!(call_hierarchy_item(&req).is_err());
    }
